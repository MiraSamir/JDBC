...
public class Parser implements IParser {
	...
	private String columnNameAlter; //The name of the column to be modified, added or dropped
	private String columnTypeAlter; //Th type of the column to be modified, added or dropped
			...
			try {
				switch (words.get(wordsIndex++).toUpperCase()) {
					...
					case "SELECT":
						...
						dbms.useDatabase(databaseName);
						if (isDistinct) {
							selected = dbms.selectDistinctFromTable(tableName, columns, where);
							operationNumber = 9;
						} else {
					...
					case "ALTER":
						dbms.useDatabase(databaseName);
						if (alter().equals("ADD")) {
							dbms.alterAdd(tableName, columns, types);
							operationNumber = 10;
						} else {
							dbms.alterDrop(tableName, columns);
							operationNumber = 11;
						} /*
	...
	private String alter() throws ParserException {
	...
	private String alterTable() throws ParserException {
		if (words.get(wordsIndex).toUpperCase().equals("ADD")) {
			...
			if (words.get(wordsIndex).equals("(")) {
				...
				throw new ParserException("The alter add query is missing an openeing bracket!");
			}
			//			alterSetValues();
			return "ADD";
